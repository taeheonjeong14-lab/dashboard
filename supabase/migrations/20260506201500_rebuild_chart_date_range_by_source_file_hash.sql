-- rebuild_chart_for_run: KPI/마스터 재계산 일자 범위를 run_id만으로 잡으면,
-- dedupe_key upsert 등으로 run_id/해시가 행마다 어긋나면 구간이 과소 평가되어 chart_daily_kpis가 소수 일만 갱신되는 문제가 있음.
-- 이 업로드 run의 source_file_hash로 잡은 일자 범위와 run_id로 잡은 범위를 함께 계산한 뒤,
-- 둘 중 더 넓은 구간(least(min들), greatest(max들))으로 합침 — 한쪽만 좁게 나와도 다른 쪽이 넓히도록 함.
