...
import { createParseRunAfterBucketing, sha256Hex } from '@/lib/chart-extraction/bucketing-run';
import { geminiGenerateText, tryParseJsonObject } from '@/lib/chart-extraction/gemini';
import { extractPdfText } from '@/lib/chart-extraction/extract-pdf-text';
...
function hasGeminiConfigured(): boolean {
  return Boolean(process.env.GEMINI_API_KEY?.trim() || process.env.NEXT_PUBLIC_GEMINI_API_KEY?.trim());
}
...
async function runGeminiBucketing(slice: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    console.warn('chart-extraction JSON parse failed (1):', firstErr);
...
async function parseMultipart(request: NextRequest): Promise<BucketingParsedInput> {
  ...
  const hospitalId = resolveHospitalInputId(formDataStringField(form, 'hospitalId'), formDataStringField(form, 'hospital_id'));
  ...
  const storageFileType = typeof storageFileTypeRaw === 'string' ? storageFileTypeRaw.trim().toLowerCase() : '';
...
async function parseJson(request: NextRequest): Promise<BucketingParsedInput> {
  ...
  if (!hospitalId || !HOSPITAL_UUID_RE.test(hospitalId)) throw new Error('INVALID_HOSPITAL_ID');
  const chartPasteText = typeof body.chartPasteText === 'string' ? body.chartPasteText.trim() : '';
  ...
  return {
    ...
    hospitalId,
    buffer,
    fileName: typeof body.fileName === 'string' && body.fileName.trim() ? body.fileName.trim() : basenameFromPath(storagePath),
    fileType,
    storageBucket,
    storagePath,
    chartPasteText,
...
export async function POST(request: NextRequest) {
  ...
  if (!hasGeminiConfigured()) {
    return NextResponse.json({ error: 'LLM provider API key가 설정되지 않았습니다. (GEMINI_API_KEY)' }, { status: 400 });
  }
  ...
  try {
    ...
    const map: Record<string, { status: number; error: string }> = {
      ...
      INVALID_CHART_TYPE: { status: 400, error: 'chartType은 intovet | plusvet | other | efriends 중 하나여야 합니다.' },
      INVALID_HOSPITAL_ID: { status: 400, error: '병원을 선택해 주세요. (hospitalId 또는 hospital_id, UUID)' },
      ...
      STORAGE_BUCKET_NOT_ALLOWED: { status: 400, error: `허용되지 않은 storage bucket입니다. (${getPdfUploadsBucket()} 만 허용)` },
      STORAGE_PATH_NOT_ALLOWED: { status: 400, error: '허용되지 않은 storage path입니다. (extract-uploads/ 접두사 필요)' },
    };
    const m = map[code];
  ...
  try {
    ...
    let { text, numPages } = await extractPdfText(input.buffer);
    if (!text) {
      return NextResponse.json(
