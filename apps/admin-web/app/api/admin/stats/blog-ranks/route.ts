...
import { fetchSummaryBlogRanks } from '@/lib/admin-stats/queries-server';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const rows = await fetchSummaryBlogRanks(gate.hospitalId);
    return NextResponse.json({ hospitalId: gate.hospitalId, rows });
  } catch (e) {
    console.error('GET /api/admin/stats/blog-ranks:', e);
