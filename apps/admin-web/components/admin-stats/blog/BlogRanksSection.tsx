...
import type { BlogRankSummaryRow, BlogRankTrendPoint } from "@/lib/admin-stats/queries-server";
import { dashboardBlogPreviewRequestUrl } from "@/lib/admin-stats/dashboard-api-public";
...
function TrendIcon({ trend }: { trend: -1 | 0 | 1 }) {
  if (trend > 0) {
    return (
      <span className="text-xs font-semibold text-emerald-700" title="상승">
        ↑
      </span>
    );
  }
  if (trend < 0) {
    return (
      <span className="text-xs font-semibold text-red-600" title="하락">
        ↓
      </span>
    );
  }
  return (
    <span className="text-xs font-semibold text-slate-400" title="변동 없음">
      —
    </span>
  );
}
...
export default function BlogRanksSection({
  ...
  const ensureTrendLoaded = (keyword: string) => {
    ...
    fetch(
      `/api/admin/stats/blog-rank-trend?hospitalId=${encodeURIComponent(hospitalId)}&keyword=${encodeURIComponent(keyword)}`,
      { credentials: "include" },
    )
      .then(async (res) => {
        const data = (await res.json()) as { points?: BlogRankTrendPoint[]; error?: string };
  ...
  const renderContentCell = (entry: RankEntry) => {
    if (!entry.url) return <span className="text-slate-500">-</span>;
    const preview = previewTitles[entry.url];
    if (!preview) return <span className="text-slate-500">불러오는 중…</span>;
    ...
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="underline underline-offset-2 text-sky-700 decoration-sky-400 hover:text-sky-900 hover:decoration-sky-600 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-sky-500/60"
        title={title}
      >
  ...
  const renderTrendPopover = () => {
    ...
    return (
      <div
        className="pointer-events-none fixed z-[60] w-[250px] border border-slate-200 bg-white p-2.5 shadow-xl"
        style={{ left, top }}
      >
        <p className="text-xs font-medium text-slate-900">{hoveredRank.keyword}</p>
        <p className="mb-2 text-[11px] text-slate-500">{part.title} · 최근 6개월</p>
        {loadingTrend && <p className="text-xs text-slate-500">불러오는 중…</p>}
        {!loadingTrend && series.length < 2 && (
          <p className="text-xs italic text-slate-500">추세를 그릴 데이터가 부족합니다.</p>
        )}
        {!loadingTrend && series.length >= 2 && (
          <>
  ...
  return (
    <>
      <section
        ...
        className={`${variant === "simple" ? "" : "bg-slate-50 "}p-4 sm:p-5`}
      >
        <h2 id={headingId} className="mb-2 text-base font-semibold text-slate-900 sm:text-lg">
          {title}
        </h2>
        <p className="mb-3 text-sm text-slate-500">{description}</p>
        {loading && <p className="text-sm text-slate-600">데이터를 불러오는 중...</p>}
            ...
            {variant === "simple" ? (
                        ...
                        {PARTS.map((part) => {
                          ...
                          return (
                            <td key={`${row.keyword}:${part.id}`} className="py-1.5 px-2">
                              {rank == null ? <span className="text-slate-500">-</span> : formatRank(rank)}
                            </td>
                          );
                        })}
                      </tr>
              ...
              <div className="flex flex-col gap-4">
                {PARTS.map((part) => {
                  ...
                  return (
                    <div key={part.id}>
                      <h3 className="mb-1 text-sm font-semibold text-slate-800">{part.title}</h3>
                      {entries.length === 0 ? (
                        <p className="text-xs italic text-slate-600">표시할 키워드가 없습니다.</p>
                      ) : (
                        <div className="overflow-x-auto">
