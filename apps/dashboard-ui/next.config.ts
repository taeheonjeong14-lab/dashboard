...
function frameAncestorsHeader(): string {
  const extra = (process.env.DASHBOARD_UI_FRAME_ANCESTORS ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  const defaults = [
  ...
  return `frame-ancestors ${[...defaults, ...extra].join(" ")}`;
}
...
const nextConfig: NextConfig = {
  ...
  /** admin-web 통계 메뉴 iframe 허용 + env 로 Vercel 프리뷰 등 확장 */
  async headers() {
    return [
      {
        source: "/:path*",
        headers: [{ key: "Content-Security-Policy", value: frameAncestorsHeader() }],
      },
    ];
  },
};
