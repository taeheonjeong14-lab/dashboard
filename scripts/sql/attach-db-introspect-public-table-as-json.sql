-- ATTACH (PDF) DB — 재사용 템플릿. WITH vars 의 sch / tbl 만 고치고 한 번 실행.
-- Supabase SQL Editor 는 여러 SELECT 를 한 번에 실행하면 마지막 결과만 보이므로, 한 칸 JSON 으로 묶음.
-- 테이블별로 고정된 파일: attach-db-describe-documents.sql, attach-db-describe-parse-runs.sql 등.
--
-- 결과: introspection_json 한 칸 복사 후 에디터에서 포맷.

WITH vars AS (
  SELECT 'public'::text AS sch, 'documents'::text AS tbl
),
cols AS (
  SELECT COALESCE(
    json_agg(
