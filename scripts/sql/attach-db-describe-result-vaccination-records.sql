...
--
-- 대상: public.result_vaccination_records
-- 컬럼: id, parse_run_id, record_type, dose_order, product_name, administered_date,
--   "sign", page, row_order — FK(parse_run_id)

WITH vars AS (
  SELECT 'public'::text AS sch, 'result_vaccination_records'::text AS tbl
),
cols AS (
  SELECT COALESCE(
    json_agg(
