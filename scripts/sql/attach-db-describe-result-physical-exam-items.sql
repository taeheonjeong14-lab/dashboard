...
--
-- 대상: public.result_physical_exam_items
-- 컬럼: id, parse_run_id, chart_by_date_id, date_time, item_name, reference_range,
--   value_text, unit, raw_text, row_order, created_at — FK(parse_run_id, chart_by_date_id)

WITH vars AS (
  SELECT 'public'::text AS sch, 'result_physical_exam_items'::text AS tbl
),
cols AS (
  SELECT COALESCE(
    json_agg(
