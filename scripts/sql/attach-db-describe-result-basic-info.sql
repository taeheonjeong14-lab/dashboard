...
--
-- 대상: public.result_basic_info
-- 컬럼: id, parse_run_id, owner_name, patient_name, species, breed, birth, sex,
--   created_at, hospital_name, age — UNIQUE(parse_run_id)

WITH vars AS (
  SELECT 'public'::text AS sch, 'result_basic_info'::text AS tbl
),
cols AS (
  SELECT COALESCE(
    json_agg(
