...
--
-- 대상: public.result_lab_items
-- 컬럼: id, parse_run_id, date_time, item_name, value_text, unit, reference_range,
--   flag, row_order, created_at, raw_item_name — FK(parse_run_id)

WITH vars AS (
  SELECT 'public'::text AS sch, 'result_lab_items'::text AS tbl
),
cols AS (
  SELECT COALESCE(
    json_agg(
