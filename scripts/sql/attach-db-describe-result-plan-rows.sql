...
--
-- 대상: public.result_plan_rows
-- 컬럼: id, parse_run_id, chart_by_date_id, code, treatment_prescription, qty, unit,
--   "day", total, route, sign_id, raw_text, row_order, created_at — FK(parse_run_id, chart_by_date_id)

WITH vars AS (
  SELECT 'public'::text AS sch, 'result_plan_rows'::text AS tbl
),
cols AS (
  SELECT COALESCE(
    json_agg(
