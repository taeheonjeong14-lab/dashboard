...
--
-- 대상: public.result_chart_by_date
-- 컬럼: id, parse_run_id, date_time, body_text, plan_text, plan_detected, row_order, created_at

WITH vars AS (
  SELECT 'public'::text AS sch, 'result_chart_by_date'::text AS tbl
),
cols AS (
  SELECT COALESCE(
    json_agg(
