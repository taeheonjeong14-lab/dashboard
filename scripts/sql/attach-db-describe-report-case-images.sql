...
-- 결과는 introspection_json 한 칸만 복사하면 됨 (컬럼·제약·FK·인덱스·행 수·테이블 코멘트).
--
-- 대상: public.report_case_images
-- 컬럼: id, parse_run_id, exam_date, file_name, exam_type, radiology_sub, brief_comment,
--   has_notable_finding, storage_path, mime_type, byte_size, created_at, finding_spots,
--   finding_confidence, related_assessment_condition

WITH vars AS (
  SELECT 'public'::text AS sch, 'report_case_images'::text AS tbl
),
cols AS (
  SELECT COALESCE(
    json_agg(
