...
-- 결과 introspection_json 한 칸만 복사.
--
-- 대상: public.result_vitals
-- 컬럼: id, parse_run_id, chart_by_date_id, date_time, weight, temperature, respiratory_rate,
--   heart_rate, bp_systolic, bp_diastolic, raw_text, row_order, created_at — FK(parse_run_id, chart_by_date_id)

WITH vars AS (
  SELECT 'public'::text AS sch, 'result_vitals'::text AS tbl
),
cols AS (
  SELECT COALESCE(
    json_agg(
